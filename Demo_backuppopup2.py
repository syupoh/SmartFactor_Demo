...
class App:
    def __init__(self, window12, window_title, video_source="C:/Users/JOHN/Downloads/FabricVideo.mp4"):
        ...
        self.btn_snapshot = tkinter.Button(self.window, overrelief="solid",
                                           text="Detect", width=200, command=self.snapshot)
    ...
    def snapshot(self):
        ...
        self.window2.geometry("{0}x{1}+300+150".format(int(self.vid.width*2), int(self.vid.height)))
        self.canvas1 = tkinter.Canvas(self.window2, width=self.vid.width*2, height=self.vid.height)
        ...
        self.canvas1.create_image(0, 0, image=self.render1, anchor=tkinter.NW)
        self.canvas1.create_image(int(self.vid.width)+10, 0, image=self.render2, anchor=tkinter.NW)
